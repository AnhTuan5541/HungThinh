...
namespace HungThinh.Controllers
{
    public class VeChungToiController : Controller
    {
        ...
        public VeChungToiController(ILogger<VeChungToiController> logger, IOptions<ConnectionStrings> connection)
        {
            _logger = logger;
        ...
        [HttpPost]
        [Authorize]
        public IActionResult UpdateFile(int id, string title, string detail, string file_url, string file_name)
        {
            string functionName = ControllerContext.ActionDescriptor.ControllerName + "/" + System.Reflection.MethodBase.GetCurrentMethod().Name;
            string userid = User.FindFirstValue(ClaimTypes.Name);
            try
            {
                ...
                using var command2 = new SqlCommand("HT_GetVeChungToi", connection2) { CommandType = CommandType.StoredProcedure };
                ...
                using var connection = new SqlConnection(_connection.DefaultConnection);
                using var command = new SqlCommand("HT_UpdateVeChungToi", connection) { CommandType = CommandType.StoredProcedure };
                ...
                string resultMessage = "Cập nhật về chúng tôi thành công!";
                CommonFunction.LogInfo(_connection.DefaultConnection, userid, resultMessage, CommonFunction.SUCCESS, functionName);
                var response = new CommonResponse<Dictionary<string, object>>
                {
                    StatusCode = CommonFunction.SUCCESS,
